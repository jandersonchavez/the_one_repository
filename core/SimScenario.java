...
public class SimScenario implements Serializable {
	...
	/** Nível aleatório de bateria para a simulação */
	public double batteryLevel; //Random battery level
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			double currentBatteryLevel = getRandomBatteryLevel();
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						...
						mmProto, mRouterProto, currentBatteryLevel);
				hosts.add(host);
			}
		}
	}
	...
	/**
	 * Returns a Randomic Battery Level
	 * @return the randomic battery level
	 */
	public World getWorld() {
		return this.world;
	}

	public double getRandomBatteryLevel(){
		Random random = new Random();
		double minBatteryLevel = 30.0;
		double maxBatteryLevel = 100.0;

		double randomBatteryLevel = minBatteryLevel + (maxBatteryLevel - minBatteryLevel) * random.nextDouble();

		this.batteryLevel = randomBatteryLevel;
